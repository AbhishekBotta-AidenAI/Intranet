...
const UpcomingApps = () => {
    ...
    return (
        ...
        <div className={`w-full max-w-full rounded-2xl ${isChatExpanded ? 'h-full' : ''}`} style={{padding: "20px 5px 7px 14px",border:"1px solid #e1e1e1" }}>
            <div className="bg-transparent rounded-lg p-3 h-full flex flex-col">
                <div className="flex flex-col gap-2 flex-1" style={{paddingBottom:"7px"}}>
                    <div className="flex items-center justify-between">
                        <h3 className="text-[21px] font-semibold">Upcoming On The App</h3>
                    </div>

                    <div className={isChatExpanded ? "grid grid-cols-1 gap-2 items-start justify-items-start" : "grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-3 items-center justify-items-center"} style={{padding:"10px 10px 10px 0px"}}>
                        {apps.map((app, index) => (
                            <button
                                key={index}
                                className={isChatExpanded ? "flex flex-col items-start gap-2 hover:scale-105 transition-transform w-full max-w-[120px] p-2" : "flex flex-col items-center gap-1 hover:scale-105 transition-transform w-full"}
                                style={isChatExpanded ? {maxWidth: 120} : {maxWidth: 80}}
                            >
                                <div className={isChatExpanded ? `w-12 h-12 ${app.color} rounded-lg flex items-start justify-start text-white p-1` : `w-8 h-8 md:w-10 md:h-10 ${app.color} rounded-lg flex items-center justify-center text-white p-1`}>
                                    <img 
                                        src={app.icon} 
                                        alt={app.label}
