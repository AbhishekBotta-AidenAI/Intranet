...
// Base URL (normalized, no trailing slash) - prefer env override, otherwise use Render service
// Set `VITE_API_BASE_URL` in your dev/prod environment to override.
...
class PostsAPI {
    ...
    async removeReaction(postId: number, user: string, reaction: string) {
        const params = new URLSearchParams({ user, reaction });
        // axios delete with body: use request() and supply data
