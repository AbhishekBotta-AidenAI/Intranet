        ...
        <div className={`w-full max-w-full md:max-w-[370px] rounded-2xl ${isChatExpanded ? 'h-full' : ''}`} style={{padding: "16px 5px 12px 20px",border:"1px solid #e1e1e1" }}>
            <div className="bg-transparent rounded-lg p-3 h-full flex flex-col">
                <div className="flex flex-col gap-2 flex-1" style={{paddingBottom:"7px"}}>
                    ...
                    <div className={isChatExpanded ? "grid grid-cols-1 gap-2 items-start justify-items-center" : "grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-3 items-center justify-items-center"} style={{padding:"10px"}}>
                        {apps.map((app, index) => (
                            <button
                                key={index}
                            ...
                            >
                                <div className={isChatExpanded ? `w-12 h-12 ${app.color} rounded-lg flex items-center justify-center text-white shadow-md p-1` : `w-8 h-8 md:w-10 md:h-10 ${app.color} rounded-lg flex items-center justify-center text-white shadow-md p-1`}>
                                    <img 
                                        src={app.icon} 
                                        alt={app.label}
