...
const OrganisationEngagement = () => {
    ...
    const refreshPosts = async () => {
        ...
        try {
            ...
            // sanitize descriptions and compute image urls once
            const sanitizedPosts = postsList.map((p: any) => {
                ...
                const imageAtt = (p.attachments || []).find((a: any) => a.is_image);
                const imageSrc = imageAtt ? postsAPI.attachmentUrl(p.id, imageAtt.id) : null;

                return { ...p, sanitizedDescription: sanitized, imageSrc };
            });

            const liked: Record<number, boolean> = {};
            sanitizedPosts.forEach((p: any) => {
                liked[p.id] =
                    Array.isArray(p.liked_users) &&
                    p.liked_users.some((u: string) => String(u).toLowerCase() === currentUser.toLowerCase());
            });
    ...
    return (
        <div className="w-full min-h-screen px-6 py-6" style={{ padding: "0 32px 32px 32px", backgroundColor: '#f0F2F5' }}>
            ...
            <div className="bg-white rounded-2xl  p-6 mt-6" style={{marginTop:"30px",padding:"32px"}}>
                ...
                <div>
                    <h3 className="text-[25px] font-medium">
                        Hey, <span className="text-[#1F89EF]">{currentUser}</span>
                    </h3>
                ...
                <div className="bg-white rounded-xl p-4 mb-6 " style={{padding:"24px",marginBottom:"32px",marginTop:"32px", border: '1px solid #E1E1E1'}}>
                    <Composer currentUser={currentUser} onNewPost={(newPost: any) => setPosts(prev => [newPost, ...prev])} />
                </div>
                ...
                <div className=" space-y-6 overflow-y-auto no-scrollbar">
                    ...
                    {posts && posts.slice(0, 10).map((p: any) => {
                        const imageAtt = (p.attachments || []).find((a: any) => a.is_image);
                        const otherAtts = (p.attachments || []).filter((a: any) => !a.is_image);
                        const imgSrc = p.imageSrc ?? (imageAtt ? postsAPI.attachmentUrl(p.id, imageAtt.id) : null);
                        return (
                        <div key={p.id} className="bg-white border rounded-xl p-4 "  style={{border: '1px solid #E1E1E1' ,padding:"20px",marginBottom:"30px"}}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3"> 
                                ...
                                <div className="text-sm text-gray-500">
                                    {(() => {
                                        const seen: string[] = (p.seen_by && Array.isArray(p.seen_by)) ? p.seen_by : [];
                                        if (!seen.length) return `Seen by ${p.views_count ?? 0}`;
                                        const last = seen[seen.length - 1];
                                        return seen.length === 1 ? `Seen by ${last}` : `Seen by ${last} +${seen.length - 1}`;
                                    })()}
                                </div>
                            </div>
                            ...
                            {p.sanitizedDescription && (
                                <div className="text-sm mt-2" style={{padding:"10px 0px 10px 0px"}} dangerouslySetInnerHTML={{__html: p.sanitizedDescription}} />
                            )}
                            ...
                            <div className="flex items-center justify-between text-gray-600 mt-4 text-sm">
                                        ...
                                        onClick={async () => {
                                            ...
                                            setPosts(prev =>
                                                ...
                                                try {
                                                    if (wasLiked) {
                                                        // user is unliking
                                                        await postsAPI.removeReaction(p.id, currentUser, 'like');
                                                    } else {
                                                        await postsAPI.addReaction(p.id, currentUser, 'like');
                                                    }
                                                } catch (e) {
                                                    // rollback on failure
                                                    setLikedMap((m) => ({ ...m, [p.id]: wasLiked }));
                                ...
                                <div className="flex items-center gap-2 text-sm text-gray-600">
                                    ...
                                    <div className="text-gray-700">
                                            {(() => {
                                                const likedUsers: string[] = Array.isArray(p.liked_users) ? p.liked_users.map((u: any) => String(u).trim()) : [];
                                                const total = likedUsers.length || (p.like_count || 0) || 0;
                                                if (!total) return '';

                                                const cur = (currentUser || '').trim().toLowerCase();
                                                const others = likedUsers.filter(u => (u || '').toLowerCase() !== cur);

                                                let displayName = '';
                                                if (others.length > 0) {
                                                    displayName = others[others.length - 1];
                                                }
                                                // } else if (likedUsers.some(u => (u || '').toLowerCase() === cur)) {
                                                //     displayName = 'You';
                                                // } else {
                                                //     displayName = likedUsers.length ? likedUsers[likedUsers.length - 1] : '';
                                                // }

                                                if (!displayName) return '';
                                                return total === 1 ? displayName : `${displayName} +${total - 1}`;
                                            })()}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-3" style={{paddingTop:"20px"}}>
                                <div className="flex items-center gap-3">
                                    {renderAuthorAvatar(currentUser)}
                                    <input
                                        value={commentsMap[p.id] || ''}
                                        onChange={(e) => setCommentsMap((m) => ({ ...m, [p.id]: e.target.value }))}
                                        onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); submitComment(p.id); } }}
...
function Composer({ currentUser, onNewPost }: { currentUser: string; onNewPost?: (p: any) => void }) {
    ...
    const handlePost = async () => {
        ...
        filesRef.current.forEach((f) => {
            ...
            try {
                ...
                const newPost = await postsAPI.createPost(fd);

                // Build a sanitized and precomputed post object to pass to parent
                const prepared = {
                    ...newPost,
                    sanitizedDescription: String(newPost.description || '')
                        .replace(/<[^>]*data-preview=["']true["'][^>]*>[\s\S]*?<\/[a-z0-9]+>/gi, '')
                        .replace(/<img[^>]*src=["']data:[^"']*["'][^>]*>/gi, ''),
                    imageSrc: (newPost.attachments || []).find((a: any) => a.is_image)
                        ? postsAPI.attachmentUrl(newPost.id, (newPost.attachments || []).find((a: any) => a.is_image).id)
                        : null,
                };

                // Notify parent to insert new post
                onNewPost?.(prepared);
    ...
    const onImageSelected = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (ev) => {
            ...
            imgEl.onload = () => {
                ...
                // create compressed blob and push as File for upload
                canvas.toBlob((blob) => {
                    if (!blob) return;
                    try {
                        const compressedFile = new File([blob], file.name, { type: 'image/jpeg' });
                        filesRef.current.push(compressedFile);
                    } catch (e) {
