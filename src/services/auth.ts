...
class AuthAPI {
  ...
  async exchangeMicrosoftIdToken(idToken: string, tenantSlug: string): Promise<MicrosoftExchangeResponse> {
    const res = await client.post<MicrosoftExchangeResponse>('/api/auth/microsoft/exchange', {
      id_token: idToken,
